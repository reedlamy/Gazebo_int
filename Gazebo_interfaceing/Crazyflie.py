...
class Crazyflie:
    ...
    def __init__(self, prefix, idx, qualisys_connected):
        ...
        self.vy = 15 # Yaw angular velocity (deg/s)
...
######################################################################################################################4
                ...
                self.msg.x = x # controlling yaw and z first
                self.msg.y = y
                ...
                while (not self.emergency_land) and ((abs(z - self.ext_z) > (2 * tol)) or (abs(yaw - self.ext_yaw) > 4)) and self.is_tracking \
...
######################################################################################################################
                ...
                self.msg.z = self.ext_z #  x and y, yaw and z already complete
                self.msg.yaw = self.ext_yaw

                self.elap_time = 0.0
                self.start = time.time()

                # Command position until within specified tolerance (form of closed loop control) for XY
                while (not self.emergency_land) and (
                        (abs(x - self.ext_x) > tol) or (abs(y - self.ext_y) > tol)) and self.is_tracking \
                        and not self.is_charging:
